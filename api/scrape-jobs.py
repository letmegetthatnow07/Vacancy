...
print(f"[INFO] Project root: {project_root}", file=sys.stderr)
    ...
    merge_result = subprocess.run(
        [sys.executable, os.path.join(project_root, 'tools/schema_merge.py'), data_json_path, 'tmp/candidates.jsonl', data_json_path],
        capture_output=True,
        text=True,
        timeout=120,
        cwd=project_root
    ...
    if kv_account and kv_token and kv_namespace:
        try:
            ...
            if hr.ok:
                print(f"[OK] Health saved to KV", file=sys.stderr)
            else:
            ...
            if job_count > 0:
                ...
                if dr.ok:
                    print(f"[OK] KV saved data.json", file=sys.stderr)
                    kv_saved = True
                else:
                    print(f"[ERROR] KV data save failed: {dr.status_code} - {dr.text[:200]}", file=sys.stderr)
            else:
    ...
    try:
        os.remove('tmp/candidates.jsonl')
    except:
        pass
    ...
    return {
        'statusCode': 500,
        'body': json.dumps({'ok': False, 'error': 'timeout', 'detail': str(e)[:200]})
    }
except Exception as e:
    ...
    return {
        'statusCode': 500,
        'body': json.dumps({'ok': False, 'error': 'exception', 'detail': str(e)[:500]})
    }
